...
export default function BudgetSection({ project, onRefresh, tr }: SectionProps) {
  ...
  const txTabFiltered = useMemo(() => {
    ...
    return txs;
  }, [filteredProjectTx, txDateFrom, txDateTo]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {budgetTab === 'transactions' && (
          <>
            {/* Period filter + batch actions */}
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2 text-xs">
                <Calendar className="w-3.5 h-3.5 text-gray-400" />
                <input type="date" value={txDateFrom} onChange={e => setTxDateFrom(e.target.value)} className="border rounded px-2 py-1 text-xs" />
                <span className="text-gray-400">&mdash;</span>
                <input type="date" value={txDateTo} onChange={e => setTxDateTo(e.target.value)} className="border rounded px-2 py-1 text-xs" />
                {(txDateFrom || txDateTo) && (
                  <button onClick={() => { setTxDateFrom(''); setTxDateTo(''); }} className="text-gray-400 hover:text-gray-600 text-xs">✕</button>
                )}
              </div>
              <div className="flex items-center gap-2">
                {selectedTx.size > 0 && (
                  <button onClick={handleBatchTxDelete} className="flex items-center gap-1 px-2.5 py-1 text-xs bg-red-50 text-red-600 rounded-md hover:bg-red-100 transition font-medium">
                    <Trash2 className="w-3 h-3" />Eliminar {selectedTx.size}
                  </button>
                )}
                <span className="text-[10px] text-gray-400">{txTabFiltered.length} transacciones</span>
              </div>
            </div>
            {txTabFiltered.length === 0 ? (
              <div className="text-center py-10">
                <DollarSign className="w-10 h-10 text-gray-300 mx-auto mb-2" />
                <p className="text-sm text-gray-400">Sin transacciones registradas{(txDateFrom || txDateTo) ? ` en el per&iacute;odo seleccionado` : ''}.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-gray-100">
                    <th className="w-8 py-2 px-1">
                      <button onClick={() => {
                        if (selectedTx.size === txTabFiltered.length) setSelectedTx(new Set());
                        else setSelectedTx(new Set(txTabFiltered.map((t: any) => t?.id)));
                      }}>
                        {selectedTx.size === txTabFiltered.length && txTabFiltered.length > 0
                          ? <CheckSquare className="w-3.5 h-3.5 text-indigo-600" />
                          : <Square className="w-3.5 h-3.5 text-gray-300" />}
                      </button>
                    </th>
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">Fecha</th>
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">Tipo</th>
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">Descripci&oacute;n</th>
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">Categor&iacute;a</th>
                    <th className="text-left py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">L&iacute;nea</th>
                    <th className="text-center py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">Estado</th>
                    <th className="text-right py-2 px-2 text-[10px] uppercase tracking-wide text-gray-500 font-medium">Monto</th>
                    <th className="w-8"></th>
                  </tr></thead>
                  <tbody>
                    {txTabFiltered.map((t: any) => {
                      const linkedLine = lines.find(l => l.id === t?.budgetLineId);
                      const isSelected = selectedTx.has(t?.id);
                      return (
                        <tr key={t?.id} className={`border-b border-gray-50 hover:bg-gray-50 transition ${isSelected ? 'bg-indigo-50/50' : ''}`}>
                          <td className="py-2 px-1">
                            <button onClick={() => toggleTxSelect(t?.id)}>
                              {isSelected ? <CheckSquare className="w-3.5 h-3.5 text-indigo-600" /> : <Square className="w-3.5 h-3.5 text-gray-300" />}
                            </button>
                          </td>
                          <td className="py-2 px-2 text-gray-600 text-xs">{formatDate(t?.date)}</td>
