...
export type TasksBoardProps = {
  /** Hub Tools vs ATLAS — só muda o título contextual */
  variant?: 'atlas' | 'hub';
};
...
export default function TasksBoard({ variant = 'atlas' }: TasksBoardProps) {
  ...
  const [view, setView] = useState<'kanban' | 'list' | 'groups'>('list');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setForm({ title: '', description: '', projectId: '', departmentId: '', groupId: '', assigneeId: '', priority: 'MEDIUM', status: 'TODO', dueDate: '', tags: '', isRecurring: false, recurrenceMonths: '1', recurrenceCount: '1' });
    fetchTasks();
    fetchGroups();
  };
  ...
  const addSubtask = async () => {
    ...
    const body: any = {
      ...
      groupId: taskDetail.groupId || undefined,
    };
  ...
  const createGroup = async () => {
    if (!newGroupName.trim() || !activeCompanyId) return;
  ...
  const deleteGroup = async (groupId: string, name: string) => {
    if (!confirm(`${L(ml('Delete section','¿Eliminar sección','Excluir secção'))} "${name}"?`)) return;
  ...
  const ungroupedTasks = filtered.filter((t: any) => !t?.groupId);
  const tasksByGroupId = (groupId: string) => filtered.filter((t: any) => t?.groupId === groupId);

  const pageTitle =
    variant === 'hub'
      ? L(ml('Team tasks', 'Tareas del equipo', 'Tarefas da equipa'))
      : tr('nav.tasks');

  return (
    <div className="space-y-6">
      <div className={cn('flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4', variant === 'hub' && 'sm:items-end')}>
        {variant === 'hub' ? (
          <div className="text-sm text-slate-500">
            {filtered?.length ?? 0} {tr('nav.tasks').toLowerCase()}
          </div>
        ) : (
          <div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-gray-100 rounded-lg p-0.5">
            <button onClick={() => setView('kanban')} title="Kanban" className={cn('px-3 py-1.5 rounded-md text-sm font-medium transition', view === 'kanban' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500')}>
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button onClick={() => setView('list')} title="List" className={cn('px-3 py-1.5 rounded-md text-sm font-medium transition', view === 'list' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500')}>
              <List className="w-4 h-4" />
            </button>
            <button onClick={() => setView('groups')} title={L(ml('Sections','Secciones','Secções'))} className={cn('px-3 py-1.5 rounded-md text-sm font-medium transition', view === 'groups' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500')}>
      ...
      {loading ? <div className="flex justify-center py-12"><div className="w-8 h-8 border-3 border-teal-600/30 border-t-teal-600 rounded-full animate-spin" /></div> : (
        view === 'kanban' ? (
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-2 rounded-xl border border-cyan-100 bg-white p-3 sm:flex-row sm:items-center">
              <input
                value={newGroupName}
                onChange={(e) => setNewGroupName(e.target.value)}
                placeholder={L(ml('New section name…','Nombre de sección…','Nome da secção…'))}
                className="flex-1 rounded-lg border px-3 py-2 text-sm"
                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), createGroup())}
              />
              <button
                type="button"
                disabled={creatingGroup || !activeCompanyId}
                onClick={createGroup}
                className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-cyan-600 px-3 py-2 text-sm font-medium text-white hover:bg-cyan-700 disabled:opacity-50"
              >
                <Plus className="h-4 w-4" />
                {L(ml('Add section','Añadir sección','Adicionar secção'))}
              </button>
            </div>

            {(groups ?? []).map((g: any) => {
              const sectionTasks = tasksByGroupId(g.id);
              return (
                <div key={g.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                  <div className="flex items-center justify-between gap-2 border-b border-slate-100 bg-slate-50 px-4 py-3">
                    <div className="flex items-center gap-2">
                      <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: g.color || '#0891b2' }} />
                      <h3 className="font-semibold text-slate-800">{g.name}</h3>
                      <span className="rounded-full bg-slate-200 px-1.5 py-0.5 text-[10px] text-slate-600">{sectionTasks.length}</span>
                    </div>
                    <button type="button" onClick={() => deleteGroup(g.id, g.name)} className="text-slate-400 hover:text-red-600">
                    ...
                    {sectionTasks.map((t: any) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate-400">{t.assignee?.name || '—'} · {formatDate(t.dueDate)}</p>
                        </div>
                        <span className="shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ backgroundColor: getStatusColor(t.status) + '20', color: getStatusColor(t.status) }}>
                          {tr(`status.${String(t.status || '').toLowerCase()}`)}
                        </span>
                      </div>
                    ))}
                    {sectionTasks.length === 0 && (
                      <p className="px-4 py-6 text-center text-sm text-slate-400">{L(ml('No tasks in this section','Sin tareas en esta sección','Sem tarefas nesta secção'))}</p>
                    )}
                  </div>
                </div>
              );
            ...
            <div className="overflow-hidden rounded-xl border border-dashed border-slate-200 bg-white/80">
              <div className="border-b border-slate-100 px-4 py-3">
                <h3 className="font-semibold text-slate-700">{L(ml('Ungrouped','Sin sección','Sem secção'))}</h3>
              </div>
              <div className="divide-y divide-slate-50">
                {ungroupedTasks.map((t: any) => (
                  <div key={t.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
                    <button type="button" className="min-w-0 flex-1 text-left" onClick={() => openTaskDetail(t.id)}>
                      <p className="truncate text-sm font-medium text-slate-900">{t.title}</p>
                    </button>
                    <select
                      value=""
                      ...
                      onChange={(e) => {
                        if (e.target.value) moveTaskToGroup(t.id, e.target.value);
                      }}
                      className="rounded border px-2 py-1 text-xs"
                    >
                      <option value="">{L(ml('Move to…','Mover a…','Mover para…'))}</option>
                      {(groups ?? []).map((g: any) => (
                ...
                {ungroupedTasks.length === 0 && groups.length === 0 && (
                  <p className="px-4 py-8 text-center text-sm text-slate-400">
                    {L(ml('Create a section to organize tasks','Crea una sección para organizar','Cria uma secção para organizar'))}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleCreate} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{L(ml('Section','Sección','Secção'))}</label>
                  <select value={form.groupId} onChange={e => setForm({ ...form, groupId: e.target.value })} className="w-full px-3 py-2 rounded-lg border text-sm">
                    <option value="">{L(ml('No section','Sin sección','Sem secção'))}</option>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 border-b">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => { setEditingTask(true); setTaskEditForm({ title: taskDetail?.title, description: taskDetail?.description, assigneeId: taskDetail?.assigneeId || '', priority: taskDetail?.priority, dueDate: taskDetail?.dueDate ? new Date(taskDetail.dueDate).toISOString().split('T')[0] : '', estimatedHours: taskDetail?.estimatedHours || '', tags: parseTags(taskDetail?.tags).join(', '), groupId: taskDetail?.groupId || '' }); }} className="text-gray-400 hover:text-teal-600"><Edit2 className="w-4 h-4" /></button>
            ...
            <div className="p-5 space-y-5">
              ...
              {editingTask ? (
                <div className="space-y-3 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    <label className="block text-sm font-medium mb-1">{L(ml('Section','Sección','Secção'))}</label>
                    <select value={taskEditForm.groupId ?? ''} onChange={e => setTaskEditForm({ ...taskEditForm, groupId: e.target.value })} className="w-full px-3 py-2 rounded-lg border text-sm">
                      <option value="">{L(ml('No section','Sin sección','Sem secção'))}</option>
                      {(groups ?? []).map((g: any) => <option key={g.id} value={g.id}>{g.name}</option>)}
                    </select>
                  </div>
