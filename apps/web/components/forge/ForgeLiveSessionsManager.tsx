...
import { Calendar, Plus, Trash2, Radio, PlayCircle, Video, ExternalLink } from 'lucide-react';
...
export function ForgeLiveSessionsManager({ courseId, modules }: Props) {
  ...
  async function ensureMeet(sessionId: string) {
    setMeetBusyId(sessionId);
    try {
      const res = await fetch(`/api/forge/live-sessions/${sessionId}/ensure-meet`, {
        method: 'POST',
      });
      const data = await res.json();
      if (!res.ok) {
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {s.meetingUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 rounded-lg bg-sky-600 px-2.5 py-1 text-xs font-bold text-white hover:bg-sky-700"
                    >
                      Abrir sala
                    ...
                    <button
                      type="button"
                      disabled={meetBusyId === s.id}
                      onClick={() => void ensureMeet(s.id)}
                      className="inline-flex items-center gap-1 rounded-lg bg-sky-600 px-2.5 py-1 text-xs font-bold text-white hover:bg-sky-700 disabled:opacity-50"
                    >
                      <Video className="h-3 w-3" />
                      {meetBusyId === s.id ? '…' : 'Gerar sala Meet'}
                    </button>
                  )}
                </div>
