  ...
  return (
    <div className="flex min-h-dvh flex-col bg-white pb-[env(safe-area-inset-bottom)]">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-6 sm:px-6">
        ...
        {info && (
          <div className="mb-5 flex items-start justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
            <span>{info}</span>
            <button type="button" onClick={() => setInfo(null)} className="shrink-0">
              <X className="h-4 w-4" />
            </button>
          </div>
        )}
