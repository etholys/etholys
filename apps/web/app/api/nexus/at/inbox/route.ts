...
export async function GET() {
  try {
    ...
    const allClientIds = [...new Set(engagements.flatMap((e) => clientCompanyIds(e)))];
    const companyRows =
    ...
    const sectorPortfolio = new Map<
      string,
      { companies: number; contracts: number; openCases: number }
    >();
    ...
    const overdueItems = inbox.filter(
      (c) => c.dueDate && new Date(c.dueDate).getTime() < startOfToday.getTime()
    );
    ...
    console.error('[nexus/at/inbox]', e);
    return NextResponse.json(
      {
        error: e instanceof Error ? e.message : 'Error interno al cargar AT',
        services: [],
        inbox: [],
        sectorPortfolio: [],
        agenda: { overdue: [], today: [], week: [], noDate: [] },
        summary: { services: 0, openCases: 0, overdue: 0, dueToday: 0, dueThisWeek: 0 },
      },
      { status: 500 }
    );
  }
