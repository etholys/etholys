...
export async function POST(
  ...
  try {
    ...
    if (generateImages && (mode === 'images' || mode === 'video' || mode === 'magic')) {
      nextCanvas = await fillImageBlocks(nextCanvas, brand, mode, { maxImages: mode === 'magic' ? 2 : 4 });
    }
...
async function fillImageBlocks(
  ...
  const maxImages = Math.max(0, opts?.maxImages ?? 4);
  let filled = 0;
  ...
  for (const page of pages) {
    for (let i = 0; i < page.blocks.length; i++) {
      if (filled >= maxImages) return { ...canvas, pages };
